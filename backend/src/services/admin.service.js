...
async function setDriverStatus(id, status, adminUserId, reason = null) {
  await getDriverRequestDetail(id);
  ...
  try {
    ...
    await connection.execute(
      ...
      WHERE id = ?
      `,
      [status, adminUserId, reason || null, id]
    );

    /*
      IMPORTANT:
      Do NOT update users.role.
      Do NOT update users.account_status.
      A user remains a normal user even after becoming an approved driver.
    */
