...
export async function findUserByEmail(email) {
  const [rows] = await pool.execute(
    `SELECT id, full_name, business_name, email, phone, password_hash, role, account_status, is_active, last_login_at, created_at, updated_at
     FROM users
     WHERE email = ?
     LIMIT 1`,
...
export async function findUserById(id) {
  const [rows] = await pool.execute(
    `SELECT id, full_name, business_name, email, phone, role, account_status, is_active, last_login_at, created_at, updated_at
     FROM users
     WHERE id = ?
     LIMIT 1`,
    [id]
