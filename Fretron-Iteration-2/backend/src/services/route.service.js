...
export async function createRoutePost(driverUserId, payload) {
  ...
  const [result] = await pool.execute(
    `INSERT INTO route_posts
      (driver_user_id, route_code, origin_city, destination_city, departure_datetime, estimated_arrival,
       available_capacity_kg, vehicle_type, pricing_model, price_amount, restrictions, operational_notes, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      driverUserId,
      routeCode,
