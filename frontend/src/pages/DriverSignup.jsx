...
export default function DriverSignup() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const data = await apiRequest("/applications/drivers/apply", {
        method: "POST",
        body: payload,
      });

      setSuccess(data.message || "Driver application submitted successfully.");
      setTimeout(() => navigate("/driver-status"), 1200);
    } catch (err) {
      ...
      <header className="border-b border-slate-200/80 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            <Link to="/driver-status" className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-lg shadow-blue-600/20 transition hover:bg-blue-700">Check status</Link>
          </div>
        </div>
      </header>
