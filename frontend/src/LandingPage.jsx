...
export default function FretronLandingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 mesh-gradient page-fade">

      {/* ─── HEADER ───────────────────────────────────────────────────────── */}
      <header className="sticky top-0 z-50 glass-header border-b border-white/20 dark:border-slate-800/60 shadow-[0_10px_30px_-22px_rgba(15,23,42,0.35)]">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3.5 lg:px-8">
          ...
          <nav className="hidden items-center gap-7 lg:flex">
            {navLinks.map((item) => (
              <a key={item.label} href={item.href} className="text-sm font-medium text-slate-600 dark:text-slate-200 transition hover:text-slate-900 dark:hover:text-white">
                {item.label}
              </a>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-2">
            <button onClick={toggleDark} aria-label="Toggle dark mode"
              className="rounded-xl border border-slate-200/70 dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400 transition hover:bg-white/70 dark:hover:bg-slate-800/70">
            ...
            <Link to="/user-login" className="fretron-btn hidden rounded-xl border border-slate-300 dark:border-slate-700 bg-white/70 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 transition hover:bg-white dark:hover:bg-slate-800 md:inline-flex">
              Login
            </Link>
            <Link to="/user-signup" className="fretron-btn rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-blue-600/25 transition hover:bg-blue-700">
              Sign Up
            </Link>
        ...
        {/* Mobile nav panel */}
        {menuOpen && (
          <div className="border-t border-white/40 dark:border-slate-800 bg-white/80 dark:bg-slate-950/90 px-4 pb-5 pt-3 lg:hidden">
            <nav className="grid gap-1">
              {navLinks.map((item) => (
                <a key={item.label} href={item.href} onClick={() => setMenuOpen(false)}
                  className="rounded-xl px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-white/60 dark:hover:bg-slate-800/70">
                  {item.label}
                </a>
              ))}
            </nav>
            <div className="mt-4 grid grid-cols-2 gap-2">
              <Link to="/user-login" onClick={() => setMenuOpen(false)} className="fretron-btn rounded-xl border border-slate-300 dark:border-slate-700 bg-white/70 py-2.5 text-center text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-800">Login</Link>
              <Link to="/user-signup" onClick={() => setMenuOpen(false)} className="fretron-btn rounded-xl bg-blue-600 py-2.5 text-center text-sm font-semibold text-white hover:bg-blue-700">Sign Up</Link>
              <Link to="/user-signup?redirect=/driver-signup" onClick={() => setMenuOpen(false)} className="fretron-btn col-span-2 rounded-xl border border-blue-200 dark:border-blue-900 bg-blue-50/70 dark:bg-blue-950/40 py-2.5 text-center text-sm font-semibold text-blue-700 dark:text-blue-400">Join as Driver</Link>
            </div>
          </div>
        )}
      </header>
      ...
      <main className="relative z-10">
        ...
        <section id="home" className="relative overflow-hidden">
          {/* Gradient mesh */}
          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_70%_50%_at_0%_0%,rgba(37,99,235,0.12),transparent)] dark:bg-[radial-gradient(ellipse_70%_50%_at_0%_0%,rgba(37,99,235,0.08),transparent)]" />
          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_40%_60%_at_105%_105%,rgba(14,165,233,0.09),transparent)]" />
          <div className="pointer-events-none absolute -right-20 top-10 h-72 w-72 rounded-full bg-blue-400/10 blur-3xl animate-orb" />
          <div className="pointer-events-none absolute -left-16 bottom-8 h-56 w-56 rounded-full bg-emerald-300/10 blur-3xl animate-orb-alt" />
          ...
          <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-14 lg:grid-cols-2 lg:items-center lg:gap-14 lg:px-8 lg:py-24">
            ...
            <div className="max-w-xl">
              <div className="animate-fade-up mb-5 inline-flex items-center gap-2 rounded-full glass glass-shimmer px-4 py-1.5 text-sm text-slate-700 dark:text-slate-100 shadow-sm">
              ...
              <p className="animate-fade-up animate-delay-200 mt-5 text-base leading-7 text-slate-800 dark:text-slate-100 sm:text-lg">
              ...
              <div className="animate-fade-up animate-delay-300 mt-6 flex items-center gap-3">
                ...
                <div className="relative flex-1 route-line-animated">
                  <div className="h-0.5 w-full rounded-full bg-gradient-to-r from-emerald-400/30 via-blue-500/40 to-rose-400/30" />
                </div>
                <div className="flex flex-col items-center gap-1">
              ...
              <div className="animate-fade-up animate-delay-400 mt-7 flex flex-col gap-3 sm:flex-row">
                <Link to="/user-signup?redirect=/user-dashboard/post-shipment"
                  className="fretron-btn relative z-10 rounded-2xl bg-blue-600 px-6 py-3 text-center text-sm font-semibold text-white shadow-xl shadow-blue-600/25 hover:bg-blue-700">
                  Post a Shipment →
                </Link>
                <Link to="/user-login?redirect=/user-dashboard/browse-routes"
                  className="fretron-btn relative z-10 rounded-2xl border border-slate-300 dark:border-slate-700 bg-white/80 dark:bg-slate-900/70 px-6 py-3 text-center text-sm font-semibold text-slate-800 dark:text-slate-300 hover:bg-white">
                  Browse Transport
                </Link>
              </div>

              <a href="#drivers" className="animate-fade-up animate-delay-500 mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-slate-600 dark:text-slate-100 transition hover:text-blue-400">
              ...
              <div className="animate-fade-up animate-delay-600 mt-8 grid grid-cols-3 gap-3">
                {[{ v: "2-way", l: "Marketplace" }, { v: "Flexible", l: "Route & Price" }, { v: "Clear", l: "Transparent" }].map((s) => (
                  <div key={s.v} className="glass glass-hover rounded-2xl p-3 shadow-sm">
                    <div className="text-base font-black text-slate-950 dark:text-white">{s.v}</div>
                    <div className="mt-0.5 text-[11px] text-slate-600 dark:text-slate-200">{s.l}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="relative animate-slide-right animate-delay-200">
              ...
              <div className="relative overflow-hidden rounded-[28px] glass shadow-2xl">
                <img src={truckImg} alt="Truck on intercity route" className="h-[320px] w-full object-cover object-center sm:h-[400px] lg:h-[480px]" />

                <div className="glass glass-badge animate-fade-in-scale absolute left-4 top-4 rounded-2xl px-3.5 py-2.5 shadow-xl">
                ...
                <div className="glass glass-badge animate-fade-in-scale animate-delay-200 absolute right-4 top-4 rounded-2xl px-3.5 py-2.5 shadow-xl">
                ...
                <div className="glass glass-badge animate-fade-in-scale animate-delay-400 absolute bottom-4 left-4 rounded-2xl px-3.5 py-2.5 shadow-xl">
        ...
        {/* ─── CONNECTING STRIP ─────────────────────────────────────────────── */}
        <div className="relative overflow-hidden bg-slate-950/90">
          <div className="mx-auto flex max-w-7xl items-center gap-6 px-4 py-8 lg:px-8">
            <div className="hidden h-px flex-1 bg-gradient-to-r from-transparent via-blue-400/20 to-transparent sm:block" />
            <div className="glass glass-badge mx-auto rounded-full px-5 py-2 text-[11px] font-bold uppercase tracking-[0.32em] text-slate-800 dark:text-slate-50">
        ...
        {/* ─── TRUST STRIP ───────────────────────────────────────────────────── */}
        <section className="border-y border-slate-200/60 dark:border-slate-800 bg-white/50 dark:bg-slate-900/40">
          <div className="mx-auto grid max-w-7xl gap-3 px-4 py-4 sm:grid-cols-2 lg:grid-cols-4 lg:px-8">
            {["Flexible transport options", "Transparent pricing", "Easy booking flow", "Shipment tracking"].map((p) => (
              <div key={p} className="glass glass-hover flex items-center gap-3 rounded-2xl px-4 py-3.5 shadow-sm">
                <div className="h-2 w-2 flex-none rounded-full bg-blue-600" />
                <span className="text-sm font-medium text-slate-700 dark:text-slate-100">{p}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
          <div className="reveal grid grid-cols-2 gap-4 sm:grid-cols-4">
            {stats.map((s, i) => (
              <div key={s.label} className={`reveal reveal-delay-${i + 1} glass glass-hover rounded-3xl p-5 text-center shadow-sm`}>
                <div className="animate-counter-glow text-4xl font-black text-blue-600">
                  <AnimatedCounter target={s.value} suffix={s.suffix} />
                </div>
                <div className="mt-1.5 text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-200">{s.label}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* ─── HOW IT WORKS ──────────────────────────────────────────────────── */}
        <section id="how-it-works" className="mx-auto max-w-7xl px-4 pb-16 lg:px-8">
          <div className="reveal max-w-2xl">
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-700 dark:text-slate-100">How it works</p>
            ...
            <p className="mt-3 text-base leading-7 text-slate-700 dark:text-slate-100">Fretron makes cargo discovery and booking easier for businesses and individuals.</p>
          </div>

          <div className="mt-10 grid gap-5 lg:grid-cols-2 xl:grid-cols-4">
            {steps.map((step, i) => (
              <div key={step.n}
                className={`reveal reveal-delay-${i + 1} glass glass-hover group relative rounded-[22px] p-6 shadow-sm`}>
                {/* Step badge */}
                <div className="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-2xl glass-badge text-sm font-black text-blue-600 dark:text-blue-400">
                  {step.n}
                </div>
                {/* Connector (desktop) */}
                ...
                <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-200">{step.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* ─── FEATURES ──────────────────────────────────────────────────────── */}
        <section id="services" className="bg-gradient-to-b from-slate-50/60 to-blue-50/20 dark:from-slate-900/60 dark:to-slate-900">
          <div className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
            <div className="reveal max-w-2xl">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-700 dark:text-slate-100">Services</p>
            ...
            <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {features.map((f, i) => (
                <div key={f.title}
                  className={`reveal reveal-delay-${(i % 3) + 1} glass glass-hover group rounded-[22px] p-6 shadow-sm`}>
                  <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500 to-sky-500 text-xl shadow-lg shadow-blue-600/20">
                    {FEATURE_ICONS[i]}
                  </div>
                  <h3 className="text-base font-bold text-slate-950 dark:text-white">{f.title}</h3>
                  <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-200">{f.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="about" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
          <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div className="reveal">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-700 dark:text-slate-100">Why Fretron</p>
              ...
              <p className="mt-3 text-base leading-7 text-slate-700 dark:text-slate-100">Fretron gives users a transparent way to find transport while helping drivers maximize cargo utilization.</p>
              <div className="mt-7 grid gap-3">
                {benefits.map((b, i) => (
                  <div key={b} className={`reveal reveal-delay-${i + 1} glass glass-hover flex items-start gap-3.5 rounded-2xl px-5 py-3.5 shadow-sm`}>
                    ...
                    <p className="text-sm leading-6 text-slate-700 dark:text-slate-100">{b}</p>
                  </div>
                ))}
              </div>
            </div>

            <div className="reveal reveal-delay-2 relative overflow-hidden rounded-[28px] border border-slate-200/70 dark:border-slate-700 shadow-2xl" style={{minHeight: '420px', height: '420px'}}>
              <img src={warehouseImg} alt="Warehouse" className="absolute inset-0 h-full w-full object-cover object-center" />
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-950/85 via-slate-950/30 to-transparent p-6">
              ...
              <div className="glass-badge animate-fade-in-scale absolute right-4 top-4 rounded-2xl px-3.5 py-2.5 shadow-xl">
        ...
        <section id="drivers" className="bg-gradient-to-b from-white dark:from-slate-950 to-blue-50/40 dark:to-slate-900/50">
          <div className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
            <div className="reveal glass glass-hover overflow-hidden rounded-[28px] shadow-xl">
              <div className="grid gap-8 p-6 lg:grid-cols-[1.1fr_0.9fr] lg:p-10">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-700 dark:text-slate-100">For Transport Partners</p>
                  ...
                  <p className="mt-4 text-base leading-7 text-slate-700 dark:text-slate-100">Join Fretron as a transport partner, publish your route on a map, and connect with shippers. Turn unused cargo space into opportunity.</p>
                  <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                    <Link to="/user-signup?redirect=/driver-signup" className="fretron-btn rounded-2xl bg-blue-600 px-6 py-3 text-center text-sm font-semibold text-white shadow-xl shadow-blue-600/25 transition hover:bg-blue-700">Join as Driver →</Link>
                ...
                {/* Mock driver card */}
                <div className="rounded-[22px] bg-gradient-to-br from-white/60 to-blue-50/40 dark:from-slate-800/70 dark:to-blue-950/30 p-4">
                  <div className="glass rounded-[18px] p-5 shadow-sm">
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3">
                      {[
                        ...
                        <div key={l} className="glass rounded-2xl p-3.5">
      ...
      {/* ─── FOOTER ────────────────────────────────────────────────────────── */}
      <footer id="contact" className="border-t border-slate-200/60 dark:border-slate-800 bg-white/70 dark:bg-slate-950/80 backdrop-blur">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:grid-cols-2 lg:grid-cols-[1.2fr_1fr_1fr_1fr] lg:px-8">
          ...
          <div>
            ...
            <div className="mt-4 flex flex-col gap-2.5">
              <Link to="/user-signup" className="fretron-btn rounded-2xl bg-blue-600 px-5 py-2.5 text-center text-sm font-semibold text-white shadow-lg shadow-blue-600/20 hover:bg-blue-700 transition">Post a Shipment</Link>
              <Link to="/user-login?redirect=/user-dashboard/browse-routes" className="fretron-btn rounded-2xl border border-slate-300 dark:border-slate-700 bg-white/70 dark:bg-slate-900/70 px-5 py-2.5 text-center text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-800 transition">Browse Transport</Link>
